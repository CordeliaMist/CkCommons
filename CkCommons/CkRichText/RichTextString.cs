            ...
            try
            {
                foreach (string part in result)
                {
                    ...
                    if (part.StartsWith(":") && part.EndsWith(":") && part.Length > 2 && !part.Contains(" "))
                    {
                        ...
                        if (CoreTextureManager.EmoteTextures.ContainsKey(emoteName))
                            _payloads.Add(new ImagePayload(() => CoreTextureManager.EmoteTextures.GetValueOrDefault(emoteName)));
                        else
